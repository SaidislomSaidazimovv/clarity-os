...
// MUHIM: onImageUpload propini qo'shdik
function MoodboardUpload({ onImageUpload }) {
  const [images, setImages] = useState([]);
  const [isDragging, setIsDragging] = useState(false);

  // Har safar images o'zgarganda, ota komponentga (BriefPanel) ma'lumot yuboramiz
  useEffect(() => {
    if (onImageUpload) {
      // Birinchi rasmni yuboramiz (yoki butun arrayni yuborsangiz ham bo'ladi)
      // Dashboard faqat bitta rasmni ko'rsatishi uchun images[0]?.url ni yuboramiz
      onImageUpload(images.length > 0 ? images[0].url : null);
    }
  ...
  const handleFiles = (files) => {
    ...
    imageFiles.forEach((file) => {
      ...
      reader.onload = (e) => {
        ...
        // Komponent ichidagi holatni yangilash
          ...
          {images.map((image) => (
            <div key={image.id} className="relative group">
              ...
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation(); // Ota div'ning click eventini to'xtatish
